...
def test_select_jellyfin_user_id_returns_matching_user():
...
def test_select_jellyfin_user_id_returns_only_user_when_no_match():
    with sqlite3.connect(":memory:") as db:
        create_users_table(db)
...
def test_select_jellyfin_user_id_rejects_multiple_unmatched_users():
    with sqlite3.connect(":memory:") as db:
        create_users_table(db)
        db.execute("insert into Users (Id, Username, NormalizedUsername) values ('abc', 'admin', 'ADMIN')")
...
def test_upsert_jellyfin_admin_permissions_replaces_existing_rows():
    with sqlite3.connect(":memory:") as db:
        db.execute("create table Permissions (Kind integer, Permission_Permissions_Guid text, RowVersion integer, UserId text, Value integer)")
        db.execute("create table Preferences (Kind integer, Preference_Preferences_Guid text, RowVersion integer, UserId text, Value text)")
